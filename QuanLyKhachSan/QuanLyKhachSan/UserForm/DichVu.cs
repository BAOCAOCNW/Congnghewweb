...
namespace QuanLyKhachSan.UserForm
{
    public partial class DichVu : Form
    {
        ...
        private void tabledichvu_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            tabledichvu.Rows[e.RowIndex].Cells["STT"].Value = e.RowIndex + 1;
        }

        private void tabledichvu_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                txt_madichvu.Text = (tabledichvu.CurrentRow.Cells["Ma_DV"].Value.ToString());
                txt_giatien.Text = Convert.ToString(tabledichvu.CurrentRow.Cells["GIA"].Value);
                Show_CboMaloaidichvu(tabledichvu.CurrentRow.Cells["Ma_LDV"].Value.ToString());
                Show_CboDonvitinh(tabledichvu.CurrentRow.Cells["Ma_DV"].Value.ToString());
            }
        }
    }
