...
INSERT dbo.VATTU( MA_VT, TEN, GIA ) VALUES (N'VT07', N'Tivi',20000.0000)
...
INSERT dbo.KHACHHANG( MA_KH ,HOTEN ,GIOITINH ,CMT ,DIACHI ,SDT ,EMAIL) VALUES (N'KH002', N'Thành Lương', 1, N'98765432', N'Long An', N'987654567', N'minhphuong13@sedff')
...
-- TRIGGER XOA DICH VU
CREATE TRIGGER XOADV ON dbo.DICHVU INSTEAD OF DELETE
AS
...
-- thêm dịch vụ . dịch vụ từng phòng
create proc THEMDV
(
	@MAPDP char(10),
	@MADV char(10),
...
as
begin
if ((@MAPDP in (select MA_PDP from dbo.CHITIETDICHVU)) and (@MADV in (select MA_DV from dbo.CHITIETDICHVU where MA_PDP = @MAPDP)))
begin 
update dbo.CHITIETDICHVU
...
insert dbo.CHITIETDICHVU (MA_DV, MA_PDP, SOLUONG)
values (@MADV, @MAPDP, @SL)

end
end
-- thực thi

THEMDV 'PDP001', 'DV01', 1
